		...
		return {
			...
			link: function (scope, element, attrs, ngModel) {
				...
				function isSameMonth(date1, date2) {
					return ((date1 && date1.year() === date2.year()) && (date1.month() === date2.month()));
				}

				function clipDate(date) {
					if (minDate && isSameMonth(minDate,date)) {
					...
					else if (minDate && minDate.isAfter(date)) {
						return null;
					}
					else if (maxDate && isSameMonth(maxDate,date)) {
